...
$1$-form とは写像であって $\R^n$ の点に $T_p^*\R^n$ の元を対応させるものをいう。
ふつうギリシア文字で表す。
...
記号 $\bigwedge^2 T_p^*\R^n$ で次のような写像 $\varphi$ 全体を表すものとする：
...
次に $\varphi_1, \varphi_2 \in T_p^*\R^n$ に対する演算 $\wedge$ を次で定義する：
...
これにより $\varphi_1 \wedge \varphi_2 \in \bigwedge^2 T_p^*\R^n.$

----

とくに $(\mathrm dx_i)_p \wedge (\mathrm dx_j)_p \in \bigwedge^2 T_p^*\R^n.$
交代双線形性から次が成り立つ：
...
$2$-form とは写像であって $\R^n$ の点に $\bigwedge^2 T_p^*\R^n$ の元を対応させるものをいう。
これもふつうギリシア文字で表す。
...
まず $\bigwedge^rT_p^*\R^n$ を定義する。それは次のような写像 $\varphi$ 全体を表すものとする：
...
$$
\varphi_1 \wedge \dotsb \wedge \varphi_r \in \bigwedge^rT_p^*\R^n.
$$

$\bigwedge^rT_p^*\R^n$ の基底を確かめる：
...
$$
\varphi^*f \coloneqq f \circ \varphi
$$

と $\varphi^*\colon\Omega^0(\R^m) \longrightarrow \Omega^0(\R^n)$ を定義する。
...
に対して $\varphi^*\colon\Omega^r(\R^m) \longrightarrow \Omega^r(\R^n)$ を

$$
\begin{aligned}
\varphi^*\omega
\coloneqq \sum (\varphi^*f_{j_1 \dotso j_r})\,
\mathrm (\varphi^*\,\mathrm dy_{j_1}) \wedge \dotsb \wedge (\varphi^*\,\mathrm dy_{j_r}).
\end{aligned}
$$

と定義する。この $\varphi^*\omega$ を $\varphi$ による $\omega$ の引き戻しという。
...
$$
\begin{aligned}
\varphi^*\mathrm dy_1 \wedge \mathrm dy_2
&= \varphi^*\mathrm dy_1 \wedge \varphi^*\mathrm dy_2\\
...
(4) & \mathrm d\circ \varphi^* = \varphi^* \circ \mathrm d.
\end{array}
$$
