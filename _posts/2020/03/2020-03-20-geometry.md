...
FirstFandamentalForm = namedtuple(
    'FirstFandamentalForm', 'r_u, r_v, E, F, G')
u, v = symbols('u v')

def first_fundamental_form(r, u=u, v=v):
    """Compute the first fundamental form of a surface `r`."""

    r_u, r_v = diff(r, u), diff(r, v)
    return FirstFandamentalForm(r_u, r_v,
        simplify(r_u.dot(r_u)),
        simplify(r_u.dot(r_v)),
        simplify(r_v.dot(r_v)))

def print_fff(I):
    print(r'$$\bm r_u = {},\quad\bm r_v = {}.$$'.format(latex(I.r_u), latex(I.r_v)))
    print()
    print(r'$$\begin{aligned}')
    print(r'E &= {},\\'.format(latex(I.E)))
    print(r'F &= {},\\'.format(latex(I.F)))
    print(r'G &= {}.'.format(latex(I.G)))
    print(r'\end{aligned}$$')
```
...
```python
a, b, c = symbols('a b c')
ellipse = Matrix([a * cos(u) * cos(v), b * cos(v) * sin(u), c * sin(v)])
I = first_fundamental_form(ellipse)
```

$$
\bm r_u = \left[\begin{matrix}- a \sin{\left(u \right)} \cos{\left(v \right)}\\b \cos{\left(u \right)} \cos{\left(v \right)}\\0\end{matrix}\right],\quad\bm r_v = \left[\begin{matrix}- a \sin{\left(v \right)} \cos{\left(u \right)}\\- b \sin{\left(u \right)} \sin{\left(v \right)}\\c \cos{\left(v \right)}\end{matrix}\right].
$$

$$
\begin{aligned}
E &=  \left(a^{2} \sin^{2}{\left(u \right)} + b^{2} \cos^{2}{\left(u \right)}\right) \cos^{2}{\left(v \right)},\\
F &= \frac{1}{8}\left(a^{2} - b^{2}\right) \left(\cos{\left(2 u - 2 v \right)} - \cos{\left(2 u + 2 v \right)}\right),\\
G &= a^{2} \sin^{2}{\left(v \right)} \cos^{2}{\left(u \right)} + b^{2} \sin^{2}{\left(u \right)} \sin^{2}{\left(v \right)} + c^{2} \cos^{2}{\left(v \right)}.
\end{aligned}
$$
...
$$
\bm r_u = \left[\begin{matrix}- a \sin{\left(u \right)} \cosh{\left(v \right)}\\b \cos{\left(u \right)} \cosh{\left(v \right)}\\0\end{matrix}\right],\quad
\bm r_v = \left[\begin{matrix}a \cos{\left(u \right)} \sinh{\left(v \right)}\\b \sin{\left(u \right)} \sinh{\left(v \right)}\\c \cosh{\left(v \right)}\end{matrix}\right].
$$

$$
\begin{aligned}
E &= a^{2} \sin^{2}{\left(u \right)} \cosh^{2}{\left(v \right)} + b^{2} \cos^{2}{\left(u \right)} \cosh^{2}{\left(v \right)},\\
F &= - \frac{1}{4}\left(a^{2} - b^{2}\right) \sin{\left(2 u \right)} \sinh{\left(2 v \right)},\\
G &= a^{2} \cos^{2}{\left(u \right)} \sinh^{2}{\left(v \right)} + b^{2} \sin^{2}{\left(u \right)} \sinh^{2}{\left(v \right)} + c^{2} \cosh^{2}{\left(v \right)}.
\end{aligned}
$$
...
$$
\bm r_u = \left[\begin{matrix}- a \sin{\left(u \right)} \sinh{\left(v \right)}\\b \cos{\left(u \right)} \sinh{\left(v \right)}\\0\end{matrix}\right],
\quad
\bm r_v = \left[\begin{matrix}a \cos{\left(u \right)} \cosh{\left(v \right)}\\b \sin{\left(u \right)} \cosh{\left(v \right)}\\c \sinh{\left(v \right)}\end{matrix}\right].
$$

$$
\begin{aligned}
E &= \left(a^{2} \sin^{2}{\left(u \right)} + b^{2} \cos^{2}{\left(u \right)}\right) \sinh^{2}{\left(v \right)},\\
F &= - \frac{1}{4}\left(a^{2} - b^{2}\right) \sin{\left(2 u \right)} \sinh{\left(2 v \right)},\\
G &= a^{2} \cos^{2}{\left(u \right)} \cosh^{2}{\left(v \right)} + b^{2} \sin^{2}{\left(u \right)} \cosh^{2}{\left(v \right)} + c^{2} \sinh^{2}{\left(v \right)}.
\end{aligned}
$$
