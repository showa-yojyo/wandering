...
$\alpha^2$ の $\mathbb Q$ 上の最小多項式は $X^3 - 2X^2 + X - 1$ でその根は
$\alpha^2, \alpha^2\omega, \alpha^2\omega^2$ である。
次の $\mathbb Q$ 同型写像 $\varphi_1, \varphi_2$ に対して

$$
\varphi_1\colon \mathbb Q(\alpha^2) \longrightarrow \mathbb Q(\alpha^2\omega),
\quad \alpha^2 \longmapsto \alpha^2\omega,\\
\varphi_2\colon \mathbb Q(\alpha^2) \longrightarrow \mathbb Q(\alpha^2\omega^2),
\quad \alpha^2 \longmapsto \alpha^2\omega^2,\\
$$
