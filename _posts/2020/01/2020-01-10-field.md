...
有限体 $K$ は $F_p$ を部分体として含む。$K$ は $F_p$ 上の有限ベクトル空間であるので
$n \coloneqq \dim_{F_p}K$ は有限である。したがって $\lvert K \rvert = p^n.$
$\blacksquare$
...
**定理（$F_q$ の性質）**：
* $(1)$ 多項式 $X^q - X$ の $F_p$ 上の最小分解体 $F_q$ は位数 $q$ の体である。
* $(2)$ $x \in \overline F_q$ が $x^q = x$ を満たすならば $x \in F_q.$
* $(3)$ $K$ が位数 $q$ の体であるならば $F_p$ 上 $F_q$ に同型である。

**証明**：
$(1):$ 代数閉包を $\Omega \coloneqq \overline F_p$ とおく。
...
* $F_p \subset L.$ $L$ は $\Omega$ の $F_p$ を含む部分環である。
* $\alpha \in L^\times \implies \alpha^{-1} = \alpha^{q - 2} \in L.$

以上より $L$ は体であることが示された。

$F_p \subset \Omega$ が $f(X)$ の最小分解体ならば $L$ を含む。
$L$ 自身体なので $L$ が $f(X)$ の最小分解体である必要がある。
$\Box$

$(2):$ $L \subset F_q$ を示せばよいが、これは上で示した。
$\Box$
...
$X^2 + X \in F_2[X]$ は多項式としては 0 ではないが、関数として 0 という妙なものだ。
...
$p \ne 0$ ならば前節の議論により $F_p \subset K.$ 閉包をとって $\overline F_p \subset \overline K.$
任意の $n \gt 0$ に対して $F_{p^n} \subset \overline K$ が成り立つから
...
$f(X) = (X - \alpha_1)\dotsb(X - \alpha_n)$ と因数分解できると仮定する。
