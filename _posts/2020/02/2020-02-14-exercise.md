...
ここで $D$ が非可換であるという仮定から $n \gt 1.$
したがって $Z(D) \subsetneq D.$

$a \in D\setminus Z(D)$ を一つとることができる。中心化群 $C_D(a)$ を考える。
これも斜体をなす。仮定から $C_D(a) \subsetneq Z(D).$

したがって $q^d \coloneqq \lvert C_D(a)\rvert\;(1 \le d \lt n)$ をみたす
$d$ が存在する。$D$ は $C_D(a)$ 上のベクトル空間でもあるので $d \mid n.$

ここで乗法群 $D^\times$ を考える（斜体は整域であるので、$D$ から $0_D$ を除いた集合と考えてよい）。
まず $D^\times$ の類等式：

$$
\lvert D^\times \rvert = \lvert Z(D^\times) \rvert + \sum_{\ast}[D^\times : C_D(x_i)]
$$
...
ただし $\ast$ は $Z(D)$ に入っていない代表元 $x_i$ すべてにわたるものとする。

$a$ を含む共役類に含まれる元の数 $[D^\times : C_D(a)]$ は

$$
[D^\times : C_D(a)] = \frac{q^n - 1}{q - 1}.
$$
