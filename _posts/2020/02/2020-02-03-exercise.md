...
$$
\begin{aligned}
    \text{(i)}\;\mathbb Q(\sqrt{2})/\mathbb Q\quad
    \text{(ii)}\;\mathbb Q(\sqrt[5]{2})/\mathbb Q\quad
    \text{(iii)}\;\mathbb Q(\zeta_8)/\mathbb Q\quad
    \text{(iv)}\;\mathbb Q(\sqrt[5]{2}, \zeta_5)/\mathbb Q
\end{aligned}
$$
...
$\mathbb Q(\sqrt[5]{2})/\mathbb Q$ に $\zeta_5$ を添加した体なので、
...
が $\mathbb Q(\sqrt[5]{2}, \zeta_5)$ に含まれている。
$\blacksquare$
...
$$
\text{(i)}: K \coloneqq \mathbb Q(\sqrt{2}, \sqrt{-2}),\;
...
$$
K^G = \{a + d\sqrt{-4}\,|\,a, d \in \mathbb Q\} = \mathbb Q[\sqrt{-1}] = \mathbb Q(\sqrt{-1}).
$$
...
$$
\text{(ii)}:
K \coloneqq \mathbb Q(\sqrt[3]{2}, \omega),\;
G \coloneqq \langle \sigma \rangle,\;
\sigma\colon
\begin{cases}
...
\begin{aligned}
    1 & \longmapsto 1, & id.\\
    \sqrt[3]{2} &\longmapsto \sqrt[3]{2}\omega,\\
    \sqrt[3]{4} &\longmapsto (\sqrt[3]{2}\omega)^2 = \sqrt[3]{4}\omega^2,\\

    \omega &\longmapsto \omega^2,\\
    \sqrt[3]{2}\omega &\longmapsto (\sqrt[3]{2}\omega)\omega^2 = \sqrt[3]{2},\\
    \sqrt[3]{4}\omega &\longmapsto (\sqrt[3]{2}\omega)^2\omega^2 = \sqrt[3]{4}\omega, & id.\\

    \omega^2 &\longmapsto \omega^4 = \omega.\\
    \sqrt[3]{2}\omega^2 &\longmapsto (\sqrt[3]{2}\omega)\omega = \sqrt[3]{2}\omega^2, & id.\\
    \sqrt[3]{4}\omega^2 &\longmapsto (\sqrt[3]{2}\omega)^2\omega = \sqrt[3]{2}.
\end{aligned}
$$

したがって $G$ は元 $1, \sqrt[3]{4}\omega, \sqrt[3]{2}\omega^2$ を不変にする。
$\mathbb Q \subset \mathbb Q(\sqrt[3]{4}\omega) = \mathbb Q(\sqrt[3]{2}\omega^2)$ だから

$$
K^G = \mathbb Q(\sqrt[3]{4}\omega).\quad\Box
$$
...
\begin{aligned}
    ...
    \sqrt[3]{4} &\longmapsto (\sqrt[3]{2})^2 = \sqrt[3]{4},\\
...
$$
\begin{aligned}
0 = f(1/a) &= \left(\frac{1}{a}\right)^n + c_{n-1}\left(\frac{1}{a}\right)^{n-1} + \dotsb + c_1\left(\frac{1}{a}\right) + c_0\\
0 &= \frac{1}{a} + c_{n-1} + \dotsb + c_1a^{n-2} + c_0a^{n-1}\\
\therefore \frac{1}{a} &= -(c_{n-1} + \dotsb + c_1a^{n-2} + c_0a^{n-1}) \in S.\\
\therefore \forall a \in S&(a \ne 0 \implies \frac{1}{a} \in S).
\end{aligned}
$$
