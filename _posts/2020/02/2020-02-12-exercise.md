...
**検討**：有限次拡大 $K/\mathbb F_p$ は位数が $[K : \mathbb F_p]$
の巡回群を Galois 群として持つ。その生成元を $\sigma$ とすると、
$\sigma\colon \alpha \longmapsto \alpha^p$ である。

$\bar f$ の作り方から $[K : \mathbb F_p] = n$ と仮定してよい。
$K = \mathbb F_{p^n}.$

$\operatorname{Gal}(K/\mathbb F_p) = \langle \sigma \rangle.$
すると言いたいことは $\operatorname{Gal}(L/\mathbb Q)$ が
$\langle \sigma \rangle$ を部分群として持つということになる。

$f(X)$ の根と $\bar f(X)$ の根の関係があるかないか。
...
$f$ の根を $\alpha_1, \dotsc, \alpha_n$ とおくと
$\bar f$ の根は $\bar\alpha_1, \dotsc, \bar\alpha_n$ になる。
