...
* **Th 14.6**: Laplace 変換が $1/s$ を因子に含むとき
  ...
  * さらに $g(x) := \mathscr{L}^{-1}[G]\left(x\right)$ が存在するとする。
...
* **Th 14.8**: 逆変換について
  ...
  * さらに $G\left(s\right)$ の逆変換 $g(x) := \mathscr{L}^{-1}[G]\left(x\right)$ が存在して、
...
* **Th 14.9** (Heaviside)
  ...
  * $Q(x) = 0$ の根を $\lambda_1, \dotsc, \lambda_n$ とし、すべて単根である（一位の零点）とする。
  ...
  $F\left(s\right) := \dfrac{P\left(s\right)}{Q\left(s\right)} = \dfrac{A}{s - \lambda} + G\left(s\right)$ とおける。ここで
...
* **Th 14.10** (Heaviside)
  ...
  \text{ where } F_k(x) := \frac{(s - \lambda_k)^{m_k}F\left(s\right)}{Q\left(s\right)}.
  $$
...
* **E 14.4**: 二階同次線形方程式
  ...
  解： $Y\left(s\right) := \mathscr{L}[y]\left(s\right)$ とおく。方程式左辺を Laplace 変換すると：
...
* **E 14.5**: 二階非同次線形方程式
  ...
  解：
  $Y\left(s\right) := \mathscr{L}[y]\left(s\right)$ および $R\left(s\right) := \mathscr{L}[r]\left(s\right)$ とする。
  途中経緯は省略すると：
...
* **E 14.7**: 4 階同次線形方程式
  ...
  $$
  \begin{aligned}
  Y_{\pm}\left(s\right) &:= \frac{(s \pm i)^2s}{(s + i)^2(s - i)^2}\\
  &= \frac{s}{(s \pm i)^2}.\\
  \\
