...
* 小平邦彦著『岩波講座基礎数学 解析入門 IV』
  * 抜けていた問題の答案を記す。
  * 第 61 問：$K = \{(x, y, z) \,\mid\, x^\frac{2}{3} + y^\frac{2}{3} + z^\frac{2}{3} \le 1\}$ の体積
  ...
  * 第 63 問：ガンマ関数に帰着させる積分
    * 仮定
      ...
      * $q > 0, r > 0, s > 0$ を定数とする。
      * 領域 $D = \{(x, y, z)\,\mid\, x > 0, y > 0, z > 0, a < x + y + z < b\}.$
  ...
  * 第 64 問：前問の一般の次元版
    * 仮定
      ...
      * 領域 $D = \{(x_1, x_2, \dotsc, x_n)\,\mid\, x_1 > 0, \dotsc, x_n > 0, a < \sum x_n < b\}.$
      * 関数 $f\colon {[a, b]} \longrightarrow \Reals$ は $C^0$ 級かつ $\forall u \quad f(u) > 0.$
    * 結論
      $$
      \def\G#1{\varGamma(#1)}
    ...
    * 証明
      ...
      * Jacobian は次の式で得られる：
        ...
        $$
      * あとは前問と同様の展開になる。
  * 第 65 問：$\displaystyle I = \int_D\!\frac{\dx_1 \dotsm \dx_n}{(x_1^2 + \dotsb + x_n^2)^s}$ の収束・発散
    * ここで $D = \{(x_1, \dotsc, x_n)\,\mid\,x_1^2 + \dotsb x_n^2 > 1\};$ 超球の外側とする。
    * うまく変形して前問の結果を応用する。
    * 変数変換を次のように定義する：
      $$
  ...
  * 第 66 問：$\displaystyle \sum_{m_1 = 1, \dotsc, m_n = 1}^\infty \frac{1}{(m_1^2 + \dotsm + m_n^2)^s}$ の収束・発散
    * 軽装版ではこの問題はボツになっている。
    * 前問の広義積分がその答えだ。
  * 以降は[4 日前のエントリー][diary377]に続く。

[ota-02]: {{ site.baseurl }}{% link _libraries/11-ota/02-omorihigashi.md %}
[ota-03]: {{ site.baseurl }}{% link _libraries/11-ota/03-omorinishi.md %}
[diary377]: {{ site.baseurl }}{% post_url 2019/06/2019-06-11-diary %}
