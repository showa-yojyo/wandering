...
* 小平邦彦著『岩波講座基礎数学 解析入門 IV』
  ...
  * 関数列と積分
    ...
    * (Th 8.9) 順序交換の法則：
      ...
      が一様収束するならば、次が成り立つ：
      $$
      \int_K\!\lim_{m \to \infty} f_n(x_1, \dotsc, x_n)\,\mathrm{d}x_1 \dotsm \mathrm{d}x_n
      = \lim_{m \to \infty} \int_K\! f_n(x_1, \dotsc, x_n)\,\mathrm{d}x_1 \dotsm \mathrm{d}x_n.
      $$
    * (Th 8.10) Arzela の定理：
      $\R^n$ 内の容積確定な有界閉領域 $K$ で連続関数列 $\{f_m(x_1, \dotsc, x_n)\}$
      が一様に有界であれば、次が成り立つ：
  ...
  * 累次積分
    ...
    * (E 8.3) 逆二乗則ポテンシャル
      * 点 $P(\rho, 0, 0)\ (\rho > 0)$ における半径 $R$ の球体 $K$ によるポテンシャルを求める。
      $$
      \begin{aligned}
      \int_K\!\frac{\mathrm{d}x\mathrm{d}y\mathrm{d}z}{\sqrt{(x - \rho)^2 + y^2 + z^2}}
      &= \int_{-R}^{R}\!\mathrm{d}x\int_{K_x}\!\frac{\mathrm{d}y\mathrm{d}z}{\sqrt{(x - \rho)^2 + y^2 + z^2}}.
      \end{aligned}
      $$
