...
**定理**：$D$ を Euclid 整域とし、$p \in D$ を既約元であるとする。
$a, b \in D$ に対して $p$ は $ab$ を割り切るとする。

このとき $p$ は $a$ を割り切るか、あるいは $p$ は $b$ を割り切る。

**証明**：$p$ が $a$ を割り切らないならば $p$ は $b$ を割り切ることを示す。

このとき $p$ が既約元であることから $p$ と $a$ は互いに素である。
すると前補題により $p$ が $b$ を割り切る必要がある。

同様にして、$p$ が $b$ を割り切らないならば $p$ は $a$ を割り切ることが成り立つ。
$\blacksquare$
